...
const SearchOptionInfiniteScrollDoctor = ({
  options,
  isLoading,
  totalItems,
  selectedDepartment,
  ...
  useEffect(() => {
    setSearchInput(selectedItems);
    if(selectedDepartment?.id === 0 || selectedDepartment?.id === ""){
      setSearchInput("")
    }
    ...
    <div>
      <div className="relative">
        <NormalInputField
          ...
          prefix={<SearchOutlined width={88} />
          }
        ...
        {showOptions && (
          <>
            <div
              id="scrollableDiv"
              style={noDataStyle}
              className={`w-full absolute rounded-md top-[104%] left-0 z-10 ${
                listOptions?.length === 0 || listOptions?.length < 8
                  ? ""
                  : "shadow-md"
              }`}
            >
              {/*Put the scroll bar always on he bottom*/}
              <InfiniteScroll
                      ...
                      <h4
                        ...
                        className={`bg-${
                          selectedItems?.id === item?.id
                            ? "[--primary]"
                            : "[--whiteBlack]"
                        } w-full border-b-2 border-x-0 border-t-0 border-gray-100 border-solid hover:bg-${
                          selectedItems?.id === item?.id
                            ? "[--primary]"
                            : "[--secondary]"
                        }  cursor-pointer px-8 text-${
                          selectedItems?.id === item?.id
                            ? "[#fff]"
                            : "[--lightText]"
                        } py-2`}
...
export default SearchOptionInfiniteScrollDoctor;
